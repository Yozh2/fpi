...
def makeplot_residuals(residuals=[], x_name='x', y_name='y', mth_name='custom'):
    ...
    ax.set_title('Total ' + str(len(residuals)) + ' iterations')

    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid()

    ax.plot(residuals[:-1], residuals[1:], color=get_color())
    plt.show()

def makeplots_residuals(r_jacobi=[], r_seidel=[], r_sor=[], x_name='x', y_name='y'):
    # Data for plotting
